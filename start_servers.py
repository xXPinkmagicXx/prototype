...
def create_grpc_server_thread()-> threading.Thread:
    grpc_server_thread = threading.Thread(target=server.serve, daemon=True)
...
def start_grpc_server():
    ...
    # start_grpc_server()
    grpc_server_thread = create_grpc_server_thread()
    
    # print(f"Starting gRPC server in a background thread {server_thread.ident}...")
    ...
    #     while True:
    #         check_server_health(stub, uptime) 
    #         time.sleep(sleep_time_sec)  # Check every 30 seconds
    #         uptime += sleep_time_sec
